...
pAuraEffectHandler AuraEffectHandler[TOTAL_AURAS]=
{
    ...
    &AuraEffect::HandleModMeleeSpeedPct,                          //319 SPELL_AURA_319
    &AuraEffect::HandleModMeleeRangedSpeedPct,                    //320 SPELL_AURA_MOD_RANGED_ATTACK_SPEED
    &AuraEffect::HandleNULL,                                      //321 SPELL_AURA_321
    &AuraEffect::HandleNoImmediateEffect,                         //322 SPELL_AURA_INTERFERE_TARGETTING
    &AuraEffect::HandleNULL,                                      //323 SPELL_AURA_323 0 spells in 4.0.6a
    &AuraEffect::HandleModSpellCritChanceShool,                   //324 SPELL_AURA_324
    &AuraEffect::HandleNULL,                                      //325 SPELL_AURA_325 0 spells in 4.0.6a
    &AuraEffect::HandlePhase,                                     //326 SPELL_AURA_326
    &AuraEffect::HandleNULL,                                      //327 SPELL_AURA_327 0 spells in 4.0.6a
    &AuraEffect::HandleNULL,                                      //328 SPELL_AURA_328 Eclipse driver
    &AuraEffect::HandleModPowerRegenPCT,                          //329 SPELL_AURA_MOD_RUNE_REGEN_SPEED
    &AuraEffect::HandleModCanCastWhileWalking,                    //330 SPELL_AURA_CAST_WHILE_WALKING
    &AuraEffect::HandleNULL,                                      //331 SPELL_AURA_331 Weather related.
    ...
    &AuraEffect::HandleNULL,                                      //334 SPELL_AURA_334 deal damage in x range. X Aura Effect value.
    &AuraEffect::HandleNULL,                                      //335 SPELL_AURA_335 something with invisibility.
    &AuraEffect::HandleNULL,                                      //336 SPELL_AURA_MOD_FLYING_RESTRICTIONS disallow flight.
    &AuraEffect::HandleNULL,                                      //337 SPELL_AURA_MOD_VENDOR_ITEMS_PRICES Reduces the price of items from all vendors by xx%.
    &AuraEffect::HandleNULL,                                      //338 SPELL_AURA_MOD_DURABILITY_LOSS Items take xx% less durability loss when you die.
    &AuraEffect::HandleNULL,                                      //339 SPELL_AURA_INCREASE_SKILL_GAIN_CHANCE Increases the chance to gain a skill increase on tradeskills by xx%.
    &AuraEffect::HandleNULL,                                      //340 SPELL_AURA_340 Increases health gained when resurrected by a guild member by xx%.
    &AuraEffect::HandleNULL,                                      //341 SPELL_AURA_341 modify cooldown of all spells with Category value from '()'
    &AuraEffect::HandleModMeleeSpeedPct,                          //342 SPELL_AURA_MOD_TIME_BETWEEN_ATTACKS
    &AuraEffect::HandleNULL,                                      //343 SPELL_AURA_343
    &AuraEffect::HandleModDamagePercentDone,                      //344 SPELL_AURA_MOD_AUTOATTACK_DAMAGE
    &AuraEffect::HandleNULL,                                      //345 SPELL_AURA_BYPASS_ARMOR_FOR_CASTER Collosus smash.
    &AuraEffect::HandleNULL,                                      //346 SPELL_AURA_PROGRESS_BAR Shows special UI (like corrupted blood).
    &AuraEffect::HandleNULL,                                      //347 SPELL_AURA_MOD_SPELL_COOLDOWN_BY_HASTE something with haste.
    &AuraEffect::HandleNULL,                                      //348 SPELL_AURA_DEPOSIT_BONUS_MONEY_IN_GUILD_BANK_ON_LOOT increases money loot by x% and gives it to guild bank.  X Aura Effect value.
    &AuraEffect::HandleNULL,                                      //349 SPELL_AURA_MOD_CURRENCY_GAIN currency points gain increased.
    &AuraEffect::HandleNULL,                                      //350 SPELL_AURA_MOD_GATHERING_ITEMS_GAINED_PERCENT 1 spell. Increase count of items from Mining, Skinning, Herbalism, and Disenchanting.
    &AuraEffect::HandleAuraModSkill,                              //351 SPELL_AURA_351 Modify Archeology skill.
    &AuraEffect::HandleNULL,                                      //352 SPELL_AURA_352 Only 1 Worgen Form.
    &AuraEffect::HandleModCamouflage,                             //353 SPELL_AURA_MOD_CAMOUFLAGE
    &AuraEffect::HandleNULL,                                      //354 SPELL_AURA_354
    &AuraEffect::HandleNULL,                                      //355 SPELL_AURA_355
...
void AuraEffect::HandleAuraWaterWalk(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    WorldPacket data;
    if (apply)
        data.Initialize(SMSG_MOVE_SPLINE_SET_WATER_WALK, 2 + 8 + 4);    // Turn Waterwalk on.
    else
        data.Initialize(SMSG_MOVE_SPLINE_SET_LAND_WALK, 2 + 8 + 4);     // Turn Waterwalk off.
